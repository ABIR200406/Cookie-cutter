...
thresh = cv2.threshold(frameDelta, 20, 300, cv2.THRESH_BINARY)[1]
change = np.sum(thresh)
#sets the minimum confidence threshold for the detection

#INITILIZING GAME COMPONENTS
#----------------------------------------------------------------
sqr_img =Image.open("path/to/square/image.jpg").resize((256, 256)) 
 # read img\sqr (1) in the sqr_img variable
mlsa =np.array(Image.open("path/to/MLSA/image.jpg"))
...
if NotWon:
    for i in range(10):
       cv2.imshow('CookieCutter', cv2.resize(kill, (0, 0), fx=0.70, fy=0.70)) 
       #show the loss screen from the kill image read before
    while True:
        cv2.imshow('CookieCutter', cv2.resize(intro, (0, 0), fx=0.70, fy=0.70))
 ...
 win = True
break
cv2.imshow('CookieCutter', cv2.resize(winner, (0, 0), fx=0.70, fy=0.70))
cv2.waitKey(125)
 #WIN SCREEN
